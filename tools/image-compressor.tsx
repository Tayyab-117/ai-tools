...
export default function ImageCompressor(){ const [img,setImg]=useState<HTMLImageElement|null>(null); const [quality,setQuality]=useState(0.8); const [format,setFormat]=useState<'image/jpeg'|'image/webp'>('image/webp'); const canvasRef=useRef<HTMLCanvasElement>(null)
  ...
  return (<div className="space-y-4">{!img && <input type="file" accept="image/*" className="input" onChange={e=>e.target.files && handleFiles(e.target.files[0])}/>}{img && <div className="space-y-3">
    <div className="card p-4 flex flex-wrap items-center gap-3"><select className="input w-48" value={format} onChange={e=>setFormat(e.target.value as any)}>
      ...
      <label className="text-sm">Quality</label><input type="range" min={0.4} max={1} step={0.05} value={quality} onChange={e=>setQuality(parseFloat(e.target.value))}/>
      <span className="text-sm">{Math.round(quality*100)}%</span><button className="btn-primary" onClick={download}>Download</button></div>
