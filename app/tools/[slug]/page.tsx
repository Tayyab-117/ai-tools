...
function JsonLd({ tool }:{ tool:(typeof tools)[number] }){
  const data:any = { '@context':'https://schema.org','@type':'SoftwareApplication', name: tool.name, applicationCategory: 'UtilitiesApplication', operatingSystem:'Web', description: tool.tagline, offers:{'@type':'Offer', price:'0', priceCurrency:'USD'} }
  const faq = tool.seo.faq?.length ? { '@context':'https://schema.org','@type':'FAQPage', mainEntity: tool.seo.faq.map((f:any)=>({'@type':'Question', name:f.q, acceptedAnswer:{'@type':'Answer', text:f.a}})) } : null
  return (<>
